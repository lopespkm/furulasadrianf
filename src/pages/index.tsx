...
export default function Home() {
  ...
  const fetchScratchCards = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://api.raspadinhafortune.shop/v1/api/scratchcards');
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      ...
      try {
        const response = await fetch('https://api.raspadinhafortune.shop/v1/api/setting');
